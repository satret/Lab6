...
public class ExecutionService {
    ...
    public ExecutionService(CollectionController controller){
        ...
        registerCommand("remove_greater", this::removeGreater);
        registerCommand("count_greater", this::CountGreaterThanDistance);
        registerCommand("count_less", this::CountLessThanDistance);
        registerCommand("remove_distance", this::RemoveAllByDistance);
    }
    ...
    public CommandResult RemoveAllByDistance(Request<?> request){
        long distance;
        try{
            //id = (distance) request.entity;
            distance= 1;
        }
        catch (Exception exc){
            return new CommandResult(ResultStatus.ERROR, "В контроллер передан аргумент другого типа");
    ...
    public CommandResult CountGreaterThanDistance(Request<?> request) {
        boolean wasPrint = false;
        ...
        try {
            ...
            return new CommandResult(ResultStatus.ERROR, "Парсинг агрумента distance не удался.");
        }
        ...
        return new CommandResult(ResultStatus.OK, String.format("Таких элементов всего %d" + count));
    }

    public CommandResult CountLessThanDistance(Request<?> request) {
        boolean wasPrint = false;
